...
BASE_SYSTEM_PROMPT = (
    ...
    "TURN-START BEHAVIOUR: When the session first connects, stay silent. Do not greet, do not summarise, do not say 'I am ready', do not acknowledge the connection. Wait for the user to speak or for actual media (image/video frame) to arrive. Output audio tokens are billed even if no one is listening — the first token of the conversation should be in direct response to the user, not a proactive opener. The only exception is if the user explicitly asks you to greet them on connect.\n\n"
